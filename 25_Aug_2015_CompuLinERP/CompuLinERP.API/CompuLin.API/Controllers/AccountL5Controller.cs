...
{
    public class AccountL5Controller
    {
        CompuLinEntityModelEntities entities;

        public bool InsertNewCategory(ACC_CAT_L5 details)
        {
            using (entities = new CompuLinEntityModelEntities())
            {
                var query = (from info in entities.ACC_CAT_L5
                             select info);

                details.CHANGED = 0;
                details.CHANGED_DATE = DateTime.Now;

                entities.ACC_CAT_L5.Add(details);
        ...
        public bool UpdateDetails(ACC_CAT_L5 searchDetails, ACC_CAT_L5 details)
        {
            using (entities = new CompuLinEntityModelEntities())
            {
                var query = (from logInfo in entities.ACC_CAT_L5
                             ...
                             logInfo.CAT_L5 == searchDetails.CAT_L5
                             select logInfo);

                if (query.Any())
                {
                    ACC_CAT_L5 catDetails = query.First();

                    entities.ACC_CAT_L5.Remove(query.First());
                    entities.SaveChanges();
        ...
        public bool DeleteCategory(ACC_CAT_L5 searchDetails)
        {
            using (entities = new CompuLinEntityModelEntities())
            {
                var query = (from details in entities.ACC_CAT_L5
                             ...
                             details.CAT_L5 == searchDetails.CAT_L5 &&
                             details.REMOVE == 0
                             select details);

                //if (!query.Any())
                //{
                //    var query2 = (from details in entities.ACC_CAT_L5
                ...
                //    ACC_CAT_L5 catDetails = query2.First();
        ...
        public List<ACC_CAT_L5> GetAllAccountDetails(ACC_CAT_L5 searchDetails, int option)
        {
            List<ACC_CAT_L5> details = new List<ACC_CAT_L5>();
            ...
            else if (option == 2)
            {
                using (entities = new CompuLinEntityModelEntities())
                {
                    var query = (from info in entities.ACC_CAT_L5
                                 ...
                                 info.CAT_L5 == searchDetails.CAT_L5 
                                 select info);
